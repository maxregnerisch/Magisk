...
subparsers_funcs = {
    ...
    'stub': build_stub,
    'emulator': setup_avd,
    'avd_patch': patch_avd_ramdisk,
    'clean': cleanup,
    'ndk': setup_ndk
}

for action, func in subparsers_funcs.items():
    subparser = subparsers.add_parser(action, help=f"{action} description")
    if action == 'emulator':
        subparser.add_argument("-s", "--skip", action="store_true", help="skip building binaries and the app")
    elif action == 'avd_patch':
        subparser.add_argument("ramdisk", help="path to ramdisk.img")
        subparser.add_argument("-s", "--skip", action="store_true", help="skip building binaries and the app")
    elif action == 'clean':
        subparser.add_argument("target", nargs="*", help="native, cpp, rust, java, or empty to clean all")
